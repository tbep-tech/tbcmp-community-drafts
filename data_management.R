...
# ---- * METHOD 1: Equal consideration of Demographic, Economic, Household, and Environmental Burdens ----

tampabay_metrics1 <- tampabay_metrics %>%
...
tampabay_metrics1_wgs84 <- st_transform(tampabay_metrics1, crs = 4326)

# Create color palette
pal_vulnerability <- colorNumeric(palette = brewer.pal(8, "YlGnBu"), domain = tampabay_metrics1_wgs84$vulnerability)
pal_vulnerability <- colorNumeric(palette = c(brewer.pal(8, "YlGnBu"), "#000000"), domain = c(0, 8))
#pal_vulnerability <- colorNumeric(plasma(256), domain = tampabay_metrics1_wgs84$vulnerability)

# Plot the number of burdens identified, and show list of burdens upon click
map1 <- tampabay_metrics1_wgs84 %>%
  leaflet() %>%
...
community_map1 <- tampabay_metrics1_wgs84
#save(community_map1, file = 'data/community_map1.RData')
#st_write(community_map1, "data/community_map1.shp", driver = "ESRI Shapefile", delete_dsn = TRUE)
#st_write(community_map1, "data/community_map1.kml", driver = "KML", delete_dsn = TRUE)


# Save the leaflet map as an HTML file that can be shared for easy viewing outside of R
library(htmlwidgets)
#saveWidget(map1, "community_map1.html", selfcontained = TRUE)


# ---- * METHOD 2: Preferential consideration of Demographic, Economic, and Household Burdens ----

tampabay_metrics2 <- tampabay_metrics %>%
  # calculate vulnerability index based on the weights applied to the 12 vulnerability criteria
  mutate(vulnerability = 
           (burden_income*1) + 
           (burden_mobile*1) + 
...
# Convert to WGS84 for viewing with leaflet
tampabay_metrics2_wgs84 <- st_transform(tampabay_metrics2, crs = 4326)

# Create color palette
pal_vulnerability <- colorNumeric(palette = brewer.pal(8, "YlGnBu"), domain = tampabay_metrics2_wgs84$vulnerability)
pal_vulnerability <- colorNumeric(palette = c(brewer.pal(8, "YlGnBu"), "#000000"), domain = c(0, 8))
#pal_vulnerability <- colorNumeric(plasma(256), domain = tampabay_metrics2_wgs84$vulnerability)

# Plot the number of burdens identified, and show list of burdens upon click
map2 <- tampabay_metrics2_wgs84 %>%
  leaflet() %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    fillColor = ~pal_vulnerability(vulnerability),
...
# Save final community map as a RData object (R), shapefile (ArcGIS), and KML (Google Maps) 
community_map2 <- tampabay_metrics2_wgs84
#save(community_map2, file = 'data/community_map2.RData')
#st_write(community_map2, "data/community_map2.shp", driver = "ESRI Shapefile", delete_dsn = TRUE)
#st_write(community_map2, "data/community_map2.kml", driver = "KML", delete_dsn = TRUE)
...
# Save the leaflet map as an HTML file that can be shared for easy viewing outside of R
dir.create("github_map", showWarnings = FALSE)
